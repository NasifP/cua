...
To get an `EGX Robo-Advisor.exe` that runs without Python:

1. Run `install.cmd` once (it makes the `.venv` the build uses).
2. Double-click **`build_exe.cmd`**. It takes a few minutes and writes to
   `dist\`:
   - `EGX Robo-Advisor\EGX Robo-Advisor.exe`: run it in place, or copy
     the whole folder anywhere;
   - `EGX-Robo-Advisor-<version>-win64.zip`: the same folder, zipped;
   - `EGX-Robo-Advisor-Setup-<version>.exe`: an installer with Start
     menu and desktop shortcuts, made only when Inno Setup 6 is installed
     (`winget install JRSoftware.InnoSetup`). It installs for the current
     user, with no administrator prompt.

The build checks the app it made (`--role selfcheck`) and fails if a part
is missing.

The app keeps everything it writes in `%LOCALAPPDATA%\EGX Robo-Advisor`:
`.env`, `config\`, the Thndr X sign-in, logs and the bus. API keys stay in
Windows Credential Manager, so keys saved from a checkout carry over. On
first start, sign in to Thndr X again in the app's tab. Uninstalling keeps
that folder.

The exe is not code-signed, so Windows SmartScreen warns on first start
("More info" -> "Run anyway"). The screen agent that `start.cmd` drives is
not in the exe; the app uses its built-in browser.
